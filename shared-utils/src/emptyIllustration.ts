...
/** 按主题令牌重着色（不依赖 DOM） */
export function recolorEmptyIllustrationSvg(
  theme: Partial<EmptyIllustrationTheme> = {},
  baseSvg = EMPTY_ILLUSTRATION_SVG,
): string {
  const c: EmptyIllustrationTheme = { ...LIGHT_DEFAULT, ...theme }
  let recolored = baseSvg
  recolored = replaceAll(recolored, '#2D9D8A', c.pr)
  recolored = replaceAll(recolored, '#E7F5F2', c.prBg)
  recolored = replaceAll(recolored, '#515151', c.fg2)
  recolored = replaceAll(recolored, '#C6CACD', c.bd)
  recolored = replaceAll(recolored, '#E6E8EA', c.bdL)
  recolored = replaceAll(recolored, 'fill="black"', `fill="${c.fg}"`)
  recolored = replaceAll(recolored, 'stroke="black"', `stroke="${c.fg}"`)
  recolored = replaceAll(recolored, 'fill="white"', `fill="${c.srf}"`)
  recolored = replaceAll(recolored, 'stroke="white"', `stroke="${c.srf}"`)
  return recolored
}
...
export function emptyIllustrationDataUrl(
  ...
  const svg =
    theme && Object.keys(theme).length > 0
      ? recolorEmptyIllustrationSvg(theme)
