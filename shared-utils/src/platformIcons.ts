/**
 * 购物平台 / 生活服务图标 — 双端共享纯数据与 SVG 工具
 * - SHOPPING_PLATFORM_ICONS / PLATFORM_SVG_MAP
 * - isPlatformIcon / getPlatformIconSpecByKey
 * - buildPlatformIconSvgString / getPlatformIconSvgDataUrl
 *
 * 端侧渲染：
 * - PC：React.createElement（frontend/src/utils/shoppingPlatformIcons.ts）
...
/** 根据 key 取规格，未知时回退淘宝 */
export const getPlatformIconSpecByKey = (key?: string): PlatformSvgSpec => {
  if (!key) return PLATFORM_SVG_MAP.taobao
  return PLATFORM_SVG_MAP[key] || PLATFORM_SVG_MAP.taobao
}

/** 判断是否为 platform_ 前缀（分类 icon 字段约定） */
